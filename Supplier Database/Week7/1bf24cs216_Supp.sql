create database supplier_piyush;
use supplier_piyush;
...
(10001,'Acme Widget','Bangalore'),(10002,'Johns','Kolkata'),(10003,'Vimal','Mumbai'),(10004,'Reliance','Delhi');
insert into parts values
(20001,'Book','Red'),(20002,'Pen','Red'),(20003,'Pencil','Green'),(20004,'Mobile','Green'),(20005,'Charger','Black');
...
select s.sname from supplier s where not exists( select * from parts p where not exists(select * from catalog c where c.sid=s.sid and c.pid=p.pid));

select s.sname from supplier s where not exists(select * from parts p where p.color='Red' and not exists( select * from catalog c where c.sid=s.sid and c.pid=p.pid));
